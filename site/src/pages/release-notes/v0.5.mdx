...
### CEL-based MCP Authorization

If you're using MCP authorization, you can now use CEL expressions for complex authorization logic:
...
authorization:
  ...
  rules:
    - cel: |
        request.auth.jwt.claims["role"] == "admin" ||
        (request.mcp.tool == "read_data" && "reader" in request.auth.jwt.scopes)
      action: Allow
```

### Migrating from metricsRequestHeaderLabels

If you were using the deprecated `metricsRequestHeaderLabels` helm value or controller flag, update to use `metricsRequestHeaderAttributes`:

**Before (v0.4 - deprecated):**
```yaml
controller:
...
- **Enhanced reasoning model support** - Deeper integration with extended thinking and reasoning capabilities
- **Additional provider integrations** - Expanding the ecosystem of supported LLM providers
- **Advanced caching strategies** - More sophisticated prompt caching and response caching options
- **Performance optimizations** - Continued focus on reducing latency and improving throughput
